...
def assemble_audiobook(
        ...
        with progress:
            ...
            for title, chapter_path, file_path, segments in expected_chapters:
                ...
                with open(concat_list_path, "w", encoding="utf-8") as f:
                    for idx, segment_path in enumerate(available_segment_paths):
                        ...
                        try:
                            ...
                            total_duration_ms += int(seg_audio.duration_seconds * 1000)
                        except Exception:
                            pass
                        ...
                        if idx < len(available_segment_paths) - 1:
                            ...
                            total_duration_ms += int(pause_seconds * 1000)
                ...
                if index < len(sorted_chapters):
                    ...
                    total_duration_ms += int(chapter_gap_seconds * 1000)
                ...
                chapter_audios.append((title, chapter_path, total_duration_ms / 1000.0))
                progress.advance(combine_task)
    ...
    with open(concat_file, "w", encoding="utf-8") as f:
        ...
        if opening_audio_path and opening_audio_path.exists():
            ...
            opening_silence_duration = random.Random(0xDEADBEEF).uniform(2.0, 4.0)
            opening_silence = AudioSegment.silent(duration=int(opening_silence_duration * 1000))
            ...
            try:
                ...
                opening_duration_ms = int(opening_audio.duration_seconds * 1000)
                silence_ms = int(opening_silence_duration * 1000)
                current_position_ms += opening_duration_ms + silence_ms
            except Exception:
                pass  # If we can't load duration, skip offset calculation
        ...
        for idx, (title, chapter_path, duration_seconds) in enumerate(chapter_audios):
            ...
            # Record chapter marker at current position
            chapter_markers.append((current_position_ms, safe_title))
            ...
            current_position_ms += int(duration_seconds * 1000)
    ...
    shutil.copy2(workspace_path, dest_path)
