...
def _yaml_scalar(value: str) -> str:
    """Quote a value for safe use as a YAML scalar.

    EPUB-derived paths and titles were interpolated raw, so a filename
    containing ": ", a leading "-", "#", or a quote produced a per-book config
    that failed to parse or silently parsed as something else.
    """
...
def _yaml_comment(value: str) -> str:
    """Collapse a value to one line so it cannot escape a YAML comment."""
...
def create_book_config_template(
    ...
    meta = metadata or {}
    # Collapsed to a single line: these are interpolated into a `prompt_preamble: |`
    # block scalar, where an embedded newline breaks the block's indentation and
    # the rest of the prompt is reparsed as top-level YAML.
    book_name = _yaml_comment(meta.get("title") or "Unknown")
    author = _yaml_comment(meta.get("author") or "Unknown")
    publisher = _yaml_comment(meta.get("publisher") or "Unknown")
    year_of_publication = _yaml_comment(meta.get("year") or "Unknown")
    ...
    if segments_doc:
        ...
        def _inclusion_lines() -> list[str]:
            """Render one YAML inclusion list.

            The translation and audiobook lists were generated by two identical
            loops, so a fix to one (such as quoting the path) had to be repeated
            in the other — and was not.
            """
            lines: list[str] = []
            for file_path, _ in all_files:
                title = file_titles.get(file_path, "")
                title_comment = f"  # {_yaml_comment(title)}" if title else ""
                if file_path in skipped_map:
                    reason = _yaml_comment(skipped_map[file_path])
                    lines.append(
                        f"  # - {_yaml_scalar(file_path)}  # (skipped: {reason}){title_comment}"
                    )
                else:
                    lines.append(f"  - {_yaml_scalar(file_path)}{title_comment}")
            return lines

        translation_lines = _inclusion_lines()
        audiobook_lines = _inclusion_lines()
